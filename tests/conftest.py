...
def sample_briefing_data() -> BriefingData:
    return BriefingData(
        ...
        tickers=[
            TickerRow(symbol="SPY", price=745.20, change_pct=0.85, likely_driver="Broad market strength"),
            TickerRow(symbol="QQQ", price=715.30, change_pct=1.20, likely_driver="Tech sector rally"),
        ],
        news_items=[
            NewsItem(title="US Tech Stocks Surge on AI Demand", source="reuters.com", url="https://example.com/reuters-tech"),
        ],
    )
...
def sample_market_snapshot() -> MarketSnapshot:
    return MarketSnapshot(
        quotes={
            "SPY": Quote(
                symbol="SPY", current_price=745.20, open_price=742.00,
                high_price=746.50, low_price=741.00, previous_close=739.00,
                change_pct=0.84, volume=45000000, source=DataSource.FINNHUB,
                timestamp=__import__("datetime").datetime.now(),
            ),
            "QQQ": Quote(
                symbol="QQQ", current_price=715.30, open_price=708.00,
                high_price=716.80, low_price=707.00, previous_close=708.00,
                change_pct=1.03, volume=32000000, source=DataSource.FINNHUB,
                timestamp=__import__("datetime").datetime.now(),
            ),
        },
        news=[
            NewsHeadline(title="Tech Stocks Rally on AI Optimism", source="reuters.com",
                         url="https://example.com", snippet="Tech stocks up", polarity=0.6),
            NewsHeadline(title="Fed Rate Decision Looms", source="bloomberg.com",
                         url="https://example.com/fed", snippet="Fed cautious", polarity=-0.2),
        ],
        rss_items=[],
    )
